...
#include "../../../../src/map/hdmap/adapter/proto_organizer.h"
#include "../../../../src/map/hdmap/adapter/xml_parser/status.h"
...
bool OpendriveAdapter::LoadData(const std::string& filename,
  ...
  if(root_node==nullptr) {std::cout<<"nullptr "<<std::endl;}
  // header
  ...
  if (!status.ok()) {
    ...
    std::cout << "fail to parse opendrive header, " << status.error_message()
          <<std::endl;    
    return false;
  }
  ...
  if (!status.ok()) {
    ...
    std::cout << "fail to parse opendrive road, " << status.error_message() 
          <<std::endl;
  ...
  if (!status.ok()) {
    ...
    std::cout << "fail to parse opendrive junction, " << status.error_message()
          <<std::endl;    
    return false;
  }
