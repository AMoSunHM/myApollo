...
//#include "../../../modules/build/navigation.pb.h"
#include "../../../src/proto/protocpp/navigation.pb.h"
...
std::string FindFirstExist(const std::string& dir, const std::string& files) {
  ...
  std::cout << "No existing file found in " << dir << "/" << files
         << ". Fallback to first candidate as default result.";
  if(!candidates.empty()) {std::cout<< "Please specify at least one map."<<std::endl;}  
...
std::string RoutingMapFile() {
  if (FLAGS_use_navigation_mode) {
    ...
    //         "true";
    std::cout<<"routing_map file is not used when FLAGS_use_navigation_mode is "
              "true" <<std::endl;
  }
...
const HDMap& HDMapUtil::BaseMap() { return *BaseMapPtr(); }
...
const HDMap& HDMapUtil::SimMap() { return *SimMapPtr(); }
