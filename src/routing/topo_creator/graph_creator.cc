...
**/
#include "../../../src/common/configs/vehicle_config_helper.h"
#include "../../../src/common/math/math_utils.h"
#include "../../../src/map/hdmap/adapter/opendrive_adapter.h"
#include "../../../src/routing/common/routing_gflags.h"
#include "../../../src/routing/topo_creator/edge_creator.h"
#include "../../../src/routing/topo_creator/node_creator.h"
...
bool GraphCreator::Create() {
  //读取高精地图，有两种方式
  //一种是Opendrive格式，通过OpendriveAdapter
  //一种是apollo自己定义的形式

  //判断一下，如果地图文件以xml结尾，则采用第一种读取方式
  //目前皆采用第一种
  ...
  //从base_map中读取道路和lane的对应关系
  //base_map的消息结构在map.proto和map_road.proto
  //lane---road对
  ...
  for (const auto& lane : pbmap_.lane()) 
  {
    ...
    //如果从road_id_map_中找到了lane_id，则在创建节点的时候指定道路id，
    //如果没有找到，那么road_id则为空
  ...
  for (const auto& lane : pbmap_.lane()) {
    ...
    //如果不保存，则需要遍历所有节点通过lane_id来查找节点，原因为node中由lane_id
    //而lane结构中没有node_id
